    ...
    $scope.findOne = function () {
        ...
        goodsService.findOne(id).success(
            function (response) {
                ...
                $scope.entity.goodsDesc.itemImages = JSON.parse(response.goodsDesc.itemImages)  //图片
...
//添加图片到页面展示
    ...
    //  删除
    $scope.delImageFromList = function (index) {
    ...
    $scope.updateSpecAttribute = function ($event, name, value) {
        ...
        if (object != null) {
            if ($event.target.checked) {  //勾选
                ...
                if (object.attributeValue.length == 0) {  //该项取消所有 ,删除该对象
    ...
    $scope.goodsStatus = ["未审核", "已审核", "审核未通过", "关闭"];   //商品状态
    $scope.MarketableStatus = ["已下架", "已上架"];   //商品上下架状态
